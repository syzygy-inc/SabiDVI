...
| `sabidvi-wasm` | ブラウザ向けの C ABI（wasm-bindgen を使わない。SabiTeX の wasm と同じ流儀）。メモリ上のフォント束と DVI から 1 ページを RGBA8 に描き、足りないフォント名を返す（missing-file 方式）。`cargo build --release -p sabidvi-wasm --target wasm32-unknown-unknown` |
